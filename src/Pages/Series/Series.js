    ...
    const fetchMovies = async () => {
      try {
        ...
        const response = await fetch(
          `https://api.themoviedb.org/3/discover/tv?api_key=${React_APP_API_KEY}&language=en-US&sort_by=popularity.desc&page=${page}&with_genres=${genreForURL}
  `,
          options
        );
          ...
          {content &&
            content.map((c) => (
              <SingleContent
                ...
                title={c.name} // Use name for TV shows
                date={c.first_air_date} // Use first_air_date for TV shows
                media_type="tv"
